...
return_type lrw_aes_encrypt( const uint8_t in[N_BLOCK], uint8_t  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
                         ...
                         int32_t n_block, uint8_t iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    {
        xor_block(iv, in);
        if(lrw_aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
...
void lrw_aes_encrypt_128( const uint8_t in[N_BLOCK], uint8_t out[N_BLOCK],
...
void lrw_aes_encrypt_256( const uint8_t in[N_BLOCK], uint8_t out[N_BLOCK],
